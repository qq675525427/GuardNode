...
pastOnions_parser.add_argument('--threshold_prev', type=int, default=50,
                               help='If the current window is < than average*threshold/100, we have a positive match')
pastOnions_parser.add_argument('--threshold_next', type=int, default=50,
                               help='If the current window is < than average*threshold/100, we have a positive match')
pastOnions_parser.add_argument('--prev_win_size', type=int, default=6,
                               help='Number of reported measurements of the previous window')
pastOnions_parser.add_argument('--cur_win_size', type=int, default=1,
                               help='Number of reported measurements of the current window')
pastOnions_parser.add_argument('--next_win_size', type=int, default=6,
                               help='Number of reported measurements of the next window')
...
pastOnions_parser.add_argument('--filtering',
                               help='filter in flag request (can be e,g,d,m or a combinaison (e.g. g,d chooses all guard-flagged nodes)',
                               default='g')
...
relaydrop_parser.add_argument('--prev_win_size', type=int, default=12,
                              help='Number of reported measurements of the previous window')
relaydrop_parser.add_argument('--next_win_size', type=int, default=1,
                              help='Number of reported measurements of the next window')
...
relaydrop_parser.add_argument('--threshold', type=float, default=1.5,
                              help='mult coefficient of average observed bw to be positive')
...
relaydrop_parser.add_argument('--filtering',
                              help='filter in flag request (can be e,g,d,m or a combinaison (e.g. g,d chooses all guard-flagged nodes)',
                              default='g')
...
thresh_variation_parser.add_argument('--prev_win_size', type=int, default=6,
                                     help='Number of reported measurements of the previous window')
thresh_variation_parser.add_argument('--next_win_size', type=int, default=6,
                                     help='Number of reported measurements of the next window')
...
thresh_variation_parser.add_argument('--filtering',
                                     help='filter in flag request (can be e,g,d,m or a combinaison (e.g. g,d chooses all guard-flagged nodes)',
                                     default='g')
    ...
    def _check_init_windows(self, descriptor, prev_win_size, cur_win_size, next_win_size):
        ...
        if self.slice_value is None:
            ...
            if tmp_slice_value != self.slice_value:  # discard the windows in case the route has updated
                ...
                if _TESTING:
                    print(
                        "{0}.{1} has changed the history interval measurements. Discarding windows and building new ones".
...
def search_for_threshold_test(history, threshold):
    ...
    for relay in history.relays:
        ...
        if r_stat.are_windows_full() and r_stat.consweight[-1] > MIN_CONSWEIGHT_CONSIDERED:
            if compare_mean(r_stat, threshold, False) and compare_variance(r_stat, False,
                                                                           True):  # remove compare_variance to obtain results with mean only
                counter += 1
        else:
            not_full += 1
    ...
    for relay in history.relays:
        ...
        if r_stat.has_new_values() and r_stat.are_windows_full() and r_stat.consweight[-1] > MIN_CONSWEIGHT_CONSIDERED:
            ...
            if attack_started_at is not None and not is_in_range(attack_started_at, r_stat.cur_win.get_data()[0][0],
                                                                 attack_started_at + (
                                                                         history.cur_win_size + 1) * 4 * 60 * 60):
                continue
            if compare_mean(r_stat, threshold, ignore_next_win) and compare_variance(r_stat, ignore_next_win,
                                                                                     apply_variance):  # remove compare_variance to obtain results with mean only
                print("Relay {0}.{1} is positive at time {2} with previous cons_weights {3} and variance {4}".format(
                    ...
                    r_stat.consweight[len(r_stat.consweight) - (history.prev_win_size + \
                                                                history.cur_win_size + history.next_win_size):-1],
                    variance(r_stat.cur_win, r_stat.slice_value)))
    ...
    if args.subparser == "process":
        ...
        for year in range(args.start_year, args.end_year + 1):
            while ((year < args.end_year) and (month <= 12)) or \
                ...
                month += 1
            month = 1
        process_consensuses.process_consensuses(in_dirs, args.initial_descriptor_dir,
                                                args.initial_extrainfo_descriptor_dir)
        ...
        if args.subparser == "pastOnions":
            ...
            if args.attack_started_at is not None:
                attack_started_at = process_consensuses.timestamp(
                    datetime.strptime(args.attack_started_at, '%Y%m%d%H').replace(tzinfo=pytz.utc))
            else:
                attack_started_at = None
            find_relaydrop_attacked_relay(pathnames, args.prev_win_size, args.attack_win,
                                          args.next_win_size, args.threshold, attack_started_at, filtering,
                                          args.variance)
            ...
            find_threshold_wrt_objective(pathnames, args.prev_win_size, args.attack_win, args.next_win_size,
                                         args.objective, args.starting_lower_bound, args.starting_upper_bound,
                                         filtering)
