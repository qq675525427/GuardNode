    ...
    def skip_listener(path, exception):
        ...
        if isinstance(exception, FileMissing) and not os.path.exists("{0}.tar.xz".format(path)):
            ...
            if "extra" in filename:
                url = "https://collector.torproject.org/archive/relay-descriptors/extra-infos/{0}.tar.xz".format(
                    filename)
            elif "server" in filename:
                url = "https://collector.torproject.org/archive/relay-descriptors/server-descriptors/{0}.tar.xz".format(
                    filename)
            elif "consensuses" in filename:
                url = "https://collector.torproject.org/archive/relay-descriptors/consensuses/{0}.tar.xz".format(
                    filename)
            else:
        ...
        for pathname in pathnames:
            ...
            for fprint, r_stat in consensus.routers.iteritems():
                ...
                if (desc_time != 0):
                    ...
                    try:
                        ...
                        if extra_found:
                            ...
                            if extra_desc.write_history_end is not None:
                                ...
                                print(
                                    "Relay {0}.{1} has an extra_desc uncomplete: write_history_end, write_history_values: {3}".format(
                                        r_stat.fingerprint, desc.nickname, extra_desc.write_history_end,
                                        extra_desc.write_history_values))
                        else:
                    ...
                    if (desc_time_fresh == None):
                        raise ValueError(
                            'Descriptor error for {0}:{1}.\n Found  descriptor before published date {2}: {3}\nDid not find descriptor for initial hibernation status for fresh period starting {4}.'.format(
                                r_stat.nickname, r_stat.fingerprint, pub_time, desc_time, valid_after_ts))
